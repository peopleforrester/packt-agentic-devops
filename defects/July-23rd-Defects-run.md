...
| D15 | Root cause is deeper; scoped to PRD 4 | The auth-disable is in `app-config.production.yaml` and baked into the `2026-07-26` image. But the chart loads only the baked `app-config.yaml`, never `app-config.production.yaml`, so the setting (and the whole workshop Backstage config) is not in effect. Verified live: the catalog 401 persists with the new image because the config is unloaded. Fixing it needs config-load + per-cluster env wiring. See `prds/4-backstage-config-wiring.md`. |
