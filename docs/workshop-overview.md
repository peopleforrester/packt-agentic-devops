...
**The whole platform, deployed in four hours.** Twenty-nine components on a real Kubernetes cluster, GitOps-driven, observable, and governed by the end of the session. Attendees leave with a running system, not a slide deck.
...
Total: 29 components, as pinned in `components.yaml`: 18 foundation, 9 AI plane, 2 self-service.
