...
| 4 | AI gateway plane | Module 2 | Gateway API CRDs, kgateway, agentgateway with the prompt guardrail and audit access logging. Client-certificate mTLS is not configured; it is unavailable on the pinned Gateway API standard channel. The AI-plane Kyverno policies are defined here in audit mode, so the AI plane is governed from birth. |
